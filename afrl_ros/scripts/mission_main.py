...
class MissionPlan(MavrosCommon):
    ...
    def __init__(self):
        ...
        #need to send heartbeat on seperate thread to ensure PX4 says im alive
    ...
    def send_heartbeat(self):
        rate = rospy.Rate(self.ros_rate)  # Hz
    ...
    def distance_to_wp(self, lat, lon, alt):
        """alt(amsl): meters, need to refactor this as seperate module"""
        R = 6371000  # metres
    ...
    def reach_position(self, lat, lon, alt, timeout, index):
        """reach position alt(amsl): meters, timeout(int): seconds"""
        ...
        for i in xrange(timeout * self.ros_rate):
            ...
            if reached:
                rospy.loginfo(
                    ...
                    format(pos_xy_d, pos_z_d, index, i / self.ros_rate, timeout))
                break
            elif i == 0 or ((i / self.ros_rate) % 10) == 0:
...
if __name__=='__main__':
    ...
    mission_plan.run_mission()
    #rostest.rosrun(PKG, name, MissionPlan)